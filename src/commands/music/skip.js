...
export const command = {
    ...
    execute: async (interaction) => {
        ...
        if (interaction.guildId == process.env.guildId && await interaction.member.roles.cache.findKey(async role => {await interaction.channel.send(role.id); role.id.test(mod)}) === undefined && (queue.metadata.voteskip < 3 || interaction.guild.members.me.voice.channel.members.size < 5)) {
