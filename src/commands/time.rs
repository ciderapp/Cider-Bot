...
use crate::{commands::{Context, Error}, DB};
...
pub async fn time (
    ...
    let db_user: Option<crate::models::User> = DB.select(("user", user.id.0))
        .await?;
    ...
    if let Some(u) = db_user {
        if let Some(tz) = u.timezone {
            ...
            ctx.send(|b| {
                b.content(format!("The time for **{}** is {}", user.name, local_time.format("%A, %B %e, %Y %I:%M:%S %p | %T %Z").to_string()))
                    .reply(true)
            }).await?;
        } else {
            ctx.send(|b| {
                b.content("Could not find user's timezone in the database. Please tell them to set thier timezone using /settimezone")
                    .reply(true)
                    .ephemeral(true)
            }).await?;
        }
