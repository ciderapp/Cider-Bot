...
export const command = {
    ...
    execute: async (interaction) => {
        ...
        if (query && !query.startsWith('https://')) {
            query = `/v1/catalog/${storefront}/search/?term=${encodeURIComponent(query)}&platform=web&types=activities,albums,apple-curators,artists,curators,editorial-items,music-movies,music-videos,playlists,songs,stations,tv-episodes,uploaded-videos,record-labels&limit=25&relate[editorial-items]=contents&include[editorial-items]=contents&include[albums]=artists&include[songs]=artists&include[music-videos]=artists&extend=artistUrl&fields[artists]=url,name,artwork,hero&fields[albums]=artistName,artistUrl,artwork,contentRating,editorialArtwork,name,playParams,releaseDate,url&with=serverBubbles,lyricHighlights&omit[resource]=autos`;
