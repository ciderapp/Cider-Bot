import { SlashCommandBuilder, resolveColor, ChatInputCommandInteraction } from 'discord.js';
...
export const command = {
    data: new SlashCommandBuilder()
        ...
        .addBooleanOption((o) => o.setName('include-info').setDescription('Include the song/abum/artist info in the embed').setRequired(false))
        .addBooleanOption((o) => o.setName('animated-artwork').setDescription('Include the animated artwork in the embed (if available)').setRequired(false)),
    category: 'MusicKit',
    execute: async (interaction: ChatInputCommandInteraction) => {
        ...
        if (animatedArtwork && res.attributes?.editorialVideo) {
            ...
            let playlist: any = await fetch(res.attributes.editorialVideo.motionDetailSquare.video);
            playlist = Buffer.from(await playlist.arrayBuffer());
            let videos = m3u8(playlist).filter((v: any) => v.CODECS.includes('avc1'));
